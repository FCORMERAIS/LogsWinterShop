WITH parsed AS (
    SELECT
        ...
        -- Timestamp brut (utilisé uniquement pour extraire les champs temps)
        to_timestamp(
            regexp_replace(
...
SELECT
    ...
    EXTRACT(DAY    FROM log_date)::int AS day,
    EXTRACT(HOUR   FROM log_date)::int AS hour,
    EXTRACT(MINUTE FROM log_date)::int AS minute,
